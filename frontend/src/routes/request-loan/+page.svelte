<script lang="ts">
    ...
    let selectedLoanAmountSTX = $state<number>(10);
    ...
    let availableLoanOptions = $state<Array<{ amountSTX: number; label: string }>>([]);
    ...
    function calculateLoanDetails() {
        ...
        if (selectedLoanAmountSTX === 0 && availableLoanOptions.length > 0) {
            selectedLoanAmountSTX = availableLoanOptions[0].amountSTX;
        }

        const loanAmount = selectedLoanAmountSTX;
    ...
    function getAvailableLoanOptions(): Array<{ amountSTX: number; label: string }> {
        if (!creditScore) return [];

        const options: Array<{ amountSTX: number; label: string }> = [];

        if (creditScore >= 700) {
            options.push(
                { amountSTX: 10, label: 'Basic Loan - 10 STX' },
                { amountSTX: 25, label: 'Standard Loan - 25 STX' },
                { amountSTX: 50, label: 'Premium Loan - 50 STX' }
            );
            ...
            options.push({ amountSTX: 10, label: 'Basic Loan - 10 STX' });
        }

        return options;
    }

    // CORREGIDO: Solicitar préstamo
    async function requestLoan() {
        ...
        try {
            ...
            const requestedAmountMicroSTX = Math.floor(selectedLoanAmountSTX * 1000000);

            console.log('REQUEST LOAN DEBUG:', {
                creditScore,
                selectedLoanAmountSTX,
                requestedAmountMicroSTX,
                termMonths,
                isValid: requestedAmountMicroSTX >= 10000000 && requestedAmountMicroSTX <= 50000000
            });

            if (requestedAmountMicroSTX < 10000000) {
                throw new Error('Minimum loan is 10 STX');
            }

            const txId = await requestLoan(creditScore, requestedAmountMicroSTX, termMonths);

            successMessage = `Loan request submitted! Transaction ID: ${txId}. Now call "Withdraw Loan" to receive your ${selectedLoanAmountSTX.toFixed(2)} STX.`;
    ...
    async function withdrawLoan() {
        ...
        try {
            ...
            successMessage = `Loan withdrawn successfully! Transaction ID: ${txId}. You have received ${selectedLoanAmountSTX.toFixed(2)} STX.`;
    ...
    async function loadAvailableLiquidity() {
        ...
        try {
            ...
            const liquidityMicroSTX = await getAvailableLiquidity();
            availableLiquidity = liquidityMicroSTX / 1000000;
        } catch (error) {
        ...
        <div class="mx-auto max-w-2xl">
            ...
            {#if creditScore !== null}
                <div class="group relative mb-6">
                    <div
                        class="absolute inset-0 translate-x-2 translate-y-2 border-3 border-black bg-gradient-to-br from-purple-600 to-cyan-600 transition-transform group-hover:translate-x-3 group-hover:translate-y-3 sm:translate-x-3 sm:translate-y-3 sm:border-4 sm:group-hover:translate-x-4 sm:group-hover:translate-y-4"
                    ></div>
                    <div
                        class="relative border-3 border-black bg-gradient-to-br from-cyan-300 to-purple-400 p-4 transition-transform group-hover:-translate-x-1 group-hover:-translate-y-1 sm:border-4 sm:p-6"
                    >
            ...
            {#if existingLoan}
                <div class="group relative mb-6">
                    <div
                        class="absolute inset-0 translate-x-2 translate-y-2 border-3 border-black bg-gradient-to-br from-amber-600 to-orange-600 transition-transform group-hover:translate-x-3 group-hover:translate-y-3 sm:translate-x-3 sm:translate-y-3 sm:border-4 sm:group-hover:translate-x-4 sm:group-hover:translate-y-4"
                    ></div>
                    <div
                        class="relative border-3 border-black bg-gradient-to-br from-amber-300 to-orange-400 p-4 transition-transform group-hover:-translate-x-1 group-hover:-translate-y-1 sm:border-4 sm:p-6"
                    >
            ...
            <!-- Liquidity Warning -->
            {#if isEligible && !existingLoan && availableLiquidity < selectedLoanAmountSTX}
                <div class="group relative mb-6">
                    <div
                        class="absolute inset-0 translate-x-2 translate-y-2 border-3 border-black bg-gradient-to-br from-yellow-600 to-amber-600 transition-transform group-hover:translate-x-3 group-hover:translate-y-3 sm:translate-x-3 sm:translate-y-3 sm:border-4 sm:group-hover:translate-x-4 sm:group-hover:translate-y-4"
                    ></div>
                    <div
                        class="relative border-3 border-black bg-gradient-to-br from-yellow-300 to-amber-400 p-4 transition-transform group-hover:-translate-x-1 group-hover:-translate-y-1 sm:border-4 sm:p-6"
                    >
                        <div class="text-center">
                            <p class="text-sm font-bold text-black">
                                ...
                                {:else}
                                    Insufficient liquidity. Available: {availableLiquidity.toFixed(2)} STX.
                                    Required: {selectedLoanAmountSTX.toFixed(2)} STX.
                                {/if}
                            </p>
                        </div>
                    </div>
            ...
            {#if isEligible && !existingLoan && availableLiquidity >= selectedLoanAmountSTX}
                <div class="group relative">
                    ...
                    ></div>
                    <div
                        class="relative border-3 border-black bg-gradient-to-br from-cyan-300 to-purple-400 p-4 transition-transform group-hover:-translate-x-1 group-hover:-translate-y-1 sm:border-4 sm:p-6 md:p-8"
                    >
                        ...
                        <div class="mb-4 rounded bg-white/50 p-3">
                            <p class="text-sm font-bold text-black">
                                Available Liquidity: {availableLiquidity.toFixed(2)} STX
                            </p>
                        </div>
                        ...
                        <div class="mb-4">
                            ...
                            <select
                                bind:value={selectedLoanAmountSTX}
                            ...
                            >
                                {#each availableLoanOptions as option}
                                    <option value={option.amountSTX}>{option.label}</option>
                                {/each}
                            </select>
                            <div class="text-lg font-black text-black">Selected: {selectedLoanAmountSTX.toFixed(2)} STX</div>
                        </div>
                        ...
                        <div class="mb-6 space-y-2">
                            ...
                            <div class="flex justify-between text-sm font-bold text-black">
                                ...
                                <span>{(selectedLoanAmountSTX * 0.1).toFixed(6)} STX</span>
                            </div>
                        </div>
            ...
            {#if !isEligible && creditScore !== null}
                <div class="group relative">
                    <div
                        class="absolute inset-0 translate-x-2 translate-y-2 border-3 border-black bg-gradient-to-br from-red-600 to-pink-600 transition-transform group-hover:translate-x-3 group-hover:translate-y-3 sm:translate-x-3 sm:translate-y-3 sm:border-4 sm:group-hover:translate-x-4 sm:group-hover:translate-y-4"
                    ></div>
                    <div
                        class="relative border-3 border-black bg-gradient-to-br from-red-300 to-pink-400 p-4 transition-transform group-hover:-translate-x-1 group-hover:-translate-y-1 sm:border-4 sm:p-6"
                    >
                        <div class="text-center">
            ...
            {#if isLoading}
                <div class="group relative">
                    <div
                        class="absolute inset-0 translate-x-2 translate-y-2 border-3 border-black bg-gradient-to-br from-gray-600 to-gray-600 transition-transform group-hover:translate-x-3 group-hover:translate-y-3 sm:translate-x-3 sm:translate-y-3 sm:border-4 sm:group-hover:translate-x-4 sm:group-hover:translate-y-4"
                    ></div>
                    <div
                        class="relative border-3 border-black bg-gradient-to-br from-gray-300 to-gray-400 p-8 transition-transform group-hover:-translate-x-1 group-hover:-translate-y-1 sm:border-4"
                    >
                        <div class="text-center">
